...
class CmsScoringService
{
    ...
    public static function calculate(Phone $phone)
    {
        ...
        if (self::compareSensorSize($mainSensorSize, '1/1.0')) {
            $mainSensorPoints = 40;
        } // 1 inch
        elseif (self::compareSensorSize($mainSensorSize, '1/1.1')) {
            $mainSensorPoints = 38;
        } elseif (self::compareSensorSize($mainSensorSize, '1/1.2')) {
            $mainSensorPoints = 36;
        } elseif (self::compareSensorSize($mainSensorSize, '1/1.3')) {
            $mainSensorPoints = 34;
        } // Nothing Phone 3
        elseif (self::compareSensorSize($mainSensorSize, '1/1.4')) {
            $mainSensorPoints = 32;
        } elseif (self::compareSensorSize($mainSensorSize, '1/1.5')) {
            $mainSensorPoints = 30;
        } // 1/1.56 standard flagship
        elseif (self::compareSensorSize($mainSensorSize, '1/1.7')) {
            $mainSensorPoints = 24;
        } // Sony IMX
        elseif (self::compareSensorSize($mainSensorSize, '1/2.0')) {
            $mainSensorPoints = 16;
        } elseif (self::compareSensorSize($mainSensorSize, '1/2.5')) {
            $mainSensorPoints = 12;
        } // Small
        else {
            $mainSensorPoints = 8;
        ...
        if (! str_contains($mainSensorSize, '/')) {
            $mainSensorSize .= ' (Est)';
        }
        ...
        if (preg_match('/(\d+\.?\d*)\s*µm/', $specs, $matches)) {
            ...
            if ($um >= 1.4) {
                $pixelScore = 25;
                $pixelLabel = "{$um}µm (Large)";
            } elseif ($um >= 1.1) {
                $pixelScore = 20;
                $pixelLabel = "{$um}µm (Good)";
            } elseif ($um >= 0.9) {
                $pixelScore = 15;
                $pixelLabel = "{$um}µm (Average)";
            } elseif ($um >= 0.7) {
                $pixelScore = 10;
                $pixelLabel = "{$um}µm (Small)";
            } else {
                $pixelScore = 5;
                $pixelLabel = "{$um}µm (Tiny)";
            }
        } else {
            ...
            if ($mp <= 12) {
                $pixelScore = 20;
                $pixelLabel = 'Est. Large (Low MP)';
            } // e.g. iPhone 12MP
            elseif ($mp >= 200) {
                $pixelScore = 5;
                $pixelLabel = 'Est. Small (200MP)';
            } else {
                $pixelScore = 15;
                $pixelLabel = 'Est. Average';
            }
        }
        ...
        $explicitExtras = 0;
        if (! empty($phone->camera->telephoto_camera_specs)) {
            $explicitExtras++;
        }
        if (! empty($phone->camera->ultrawide_camera_specs)) {
            $explicitExtras++;
        }
        ...
        if (! empty($selfieSpecs)) {
            ...
            $apPts = 0;
            if ($selfieAperture > 0) {
                if ($selfieAperture <= 1.8) {
                    $apPts = 15;
                } elseif ($selfieAperture <= 2.0) {
                    $apPts = 12;
                } elseif ($selfieAperture <= 2.4) {
                    $apPts = 8;
                } else {
                    $apPts = 4;
                }
        ...
        $scoreResolution = function ($mp, $specsString) {
            ...
            if ($mp >= 200) {
                return ['points' => 50, 'label' => '200MP'];
            }
            if ($mp >= 108) {
                return ['points' => 44, 'label' => '108MP'];
            }
            if ($mp >= 64) {
                return ['points' => 36, 'label' => "{$mp}MP"];
            }
            if ($mp >= 50) {
                return ['points' => 30, 'label' => "{$mp}MP"];
            }
            if ($mp >= 40) {
                return ['points' => 24, 'label' => "{$mp}MP"];
            }
            if ($mp >= 32) {
                return ['points' => 20, 'label' => "{$mp}MP"];
            }
            if ($mp >= 20) {
                return ['points' => 16, 'label' => "{$mp}MP"];
            }
            if ($mp >= 12) {
                return ['points' => 12, 'label' => "{$mp}MP"];
            }
        ...
        if ($mainMP > 0) {
            $cameras[] = ['name' => 'Main', 'mp' => $mainMP, 'specs' => $mainSpecs];
        }
        ...
        if ($teleMP > 0) {
            $cameras[] = ['name' => 'Telephoto', 'mp' => $teleMP, 'specs' => $teleSpecs];
        }
        ...
        if ($uwMP > 0) {
            $cameras[] = ['name' => 'Ultrawide', 'mp' => $uwMP, 'specs' => $uwSpecs];
        }
        ...
        if ($frontMP > 0) {
            $cameras[] = ['name' => 'Front', 'mp' => $frontMP, 'specs' => $frontSpecs];
        }
        ...
        $scoreAutofocus = function ($specs, $features) {
            ...
            if ($hasLaser && $hasDualPixel) {
                return ['score' => 1.0, 'label' => 'Dual-Pixel+Laser'];
            }
            if ($hasLaser && ($hasMultiPDAF || $hasPDAF)) {
                return ['score' => 0.9, 'label' => 'Laser+Multi-PDAF'];
            }
            if ($hasDualPixel) {
                return ['score' => 0.8, 'label' => 'Dual-Pixel AF'];
            }
            if ($hasMultiPDAF) {
                return ['score' => 0.7, 'label' => 'Multi-PDAF'];
            }
            if ($hasPDAF) {
                return ['score' => 0.5, 'label' => 'PDAF'];
            }
        ...
        if (str_contains($selfieVideo, '4k')) {
            $selfieScore = 40;
            $selfieLabel = '4K Selfie Video';
        } elseif (str_contains($selfieVideo, '60fps')) {
            $selfieScore = 30;
            $selfieLabel = '1080p @ 60fps';
        }
        ...
        if (str_contains($allSpecsLow, 'periscope') && $zoomLevel >= 5) {
            $zoomScore = 60;
            $zoomLabel = '5x+ Periscope';
        } elseif (str_contains($allSpecsLow, 'periscope')) {
            $zoomScore = 50;
            $zoomLabel = 'Periscope Zoom';
        } elseif ($zoomLevel >= 3) {
            $zoomScore = 40;
            $zoomLabel = '3x+ Telephoto';
        } elseif ($zoomLevel >= 2) {
            $zoomScore = 30;
            $zoomLabel = '2x Telephoto';
        }
