    ...
    if ($phone) {
        ...
        if ($phone->cms_details && isset($phone->cms_details['resolution'])) {
            ...
            if ($phone->camera) {
                ...
                $binningKeywords = [];
                if (str_contains($allSpecs, 'quad bayer')) {
                    $binningKeywords[] = 'Quad Bayer';
                }
                if (str_contains($allSpecs, 'tetra binning')) {
                    $binningKeywords[] = 'Tetra Binning';
                }
                if (str_contains($allSpecs, 'pixel binning')) {
                    $binningKeywords[] = 'Pixel Binning';
                }
                if (str_contains($allSpecs, '4-in-1')) {
                    $binningKeywords[] = '4-in-1';
                }
                if (str_contains($allSpecs, '9-in-1')) {
                    $binningKeywords[] = '9-in-1';
                }
