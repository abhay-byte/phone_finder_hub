...
## 9. External Camera Benchmarks (CMS Scoring)

After adding the phone and its specifications, you must manually search for and add external camera benchmark scores to ensure the Camera Mastery Score (CMS) is accurate.

1.  **Search for Scores**:
    *   **DxOMark**: Search for "[Phone Name] DxOMark camera score".
    *   **PhoneArena**: Search for "[Phone Name] PhoneArena camera review" and look for the camera score.
    *   **91Mobiles**: Search for "[Phone Name] 91Mobiles review" and look for the Expert Rating (e.g., 8.5/10 -> 85).

2.  **Update Script**:
    *   Open `scripts/finalize_benchmarks.php`.
    *   Add a new section for the phone using the searched scores.
    *   If no score is found, look for a reputable tech review rating and convert to a 0-100 scale (e.g. 4/5 stars = 80).

3.  **Run the Script**:
    *   Execute the script to populate the database and recalculate the CMS score:
    ```bash
    php scripts/finalize_benchmarks.php
    ```

4.  **Recalculate All Rankings**:
    *   Finally, recalculate all scores to ensure the new phone's CMS score is reflected in the global leaderboards and rankings (FPI, Value Score, etc.):
    ```bash
    php artisan phone:recalculate-scores
    php artisan cache:clear
    ```