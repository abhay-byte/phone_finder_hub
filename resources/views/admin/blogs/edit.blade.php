...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-colors duration-300">
    <div>
        <h1 class="text-3xl font-bold text-slate-900 dark:text-white transition-colors duration-300 mb-2">Edit Blog Post</h1>
        <p class="text-slate-500 dark:text-slate-400 transition-colors duration-300">Update your content, featured image, or publishing status.</p>
    </div>
    <div class="flex items-center gap-3">
        @if($blog->is_published)
            <a href="{{ route('blogs.show', $blog->slug) }}" target="_blank" class="text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/30 hover:bg-teal-100 dark:hover:bg-teal-900/50 px-4 py-2 rounded-xl text-sm font-bold transition-colors shadow-sm flex items-center gap-2">
                View Public Post
        ...
        @endif
        <a href="{{ route('admin.blogs.index') }}" class="text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors text-sm font-medium flex items-center gap-2 px-2">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-gray-50 dark:bg-slate-800 rounded-2xl p-5 border border-gray-100 dark:border-white/5 transition-colors duration-300">
                    ...
                    <label class="relative inline-flex items-center cursor-pointer">
                        ...
                        <span class="ml-3 text-sm font-medium text-slate-900 dark:text-slate-300 transition-colors duration-300">Published</span>
                    </label>
                    <p class="text-xs text-gray-500 dark:text-slate-500 mt-2 transition-colors duration-300">
                ...
                @endphp
                <div class="bg-gray-50 dark:bg-slate-800 rounded-2xl p-5 border border-gray-100 dark:border-white/5 transition-colors duration-300" x-data="{ imageType: '{{ $isUrlImage ? 'url' : 'upload' }}' }">
                    ...
                    <div x-show="imageType === 'upload'" class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-white/10 border-dashed rounded-xl relative group overflow-hidden bg-white/50 dark:bg-slate-900/50" id="imageArea">
                        ...
                        <div class="space-y-1 text-center relative z-10 p-2 {{ !$isUrlImage && $blog->featured_image ? 'bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity' : '' }}" id="imagePromptUpload">
                    ...
                    <div x-show="imageType === 'url'" class="space-y-4" style="display: none;">
                        ...
                        <div class="mt-4 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-white/10 border-dashed rounded-xl relative overflow-hidden bg-white/50 dark:bg-slate-900/50" id="imageUrlArea" style="min-height: 150px;">
                            ...
                            <span id="imageUrlPlaceholder" class="text-gray-400 dark:text-gray-500 text-sm relative z-10 my-auto {{ $isUrlImage && $blog->featured_image ? 'hidden' : '' }}">Image preview will appear here</span>
                        </div>
    ...
    function previewImageUpload(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                prompt.classList.add('bg-white/80', 'dark:bg-slate-900/80', 'backdrop-blur-sm', 'opacity-0', 'group-hover:opacity-100', 'transition-opacity');
            }
