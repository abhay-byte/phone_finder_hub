            @forelse($blogs as $blog)
                <a href="{{ route('blogs.show', $blog->slug) }}" class="group flex flex-col bg-white dark:bg-slate-900 rounded-3xl border border-gray-200 dark:border-white/5 overflow-hidden shadow-sm hover:shadow-xl hover:border-teal-200 dark:hover:border-teal-900/50 transition-all duration-300 transform hover:-translate-y-1">
                    <div class="aspect-video w-full bg-gray-100 dark:bg-slate-800 relative overflow-hidden">
                    ...
                    <div class="p-6 flex flex-col flex-1">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <span class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide">{{ $blog->author->name ?? 'Guest Author' }}</span>
                            <span class="w-1 h-1 rounded-full bg-gray-300 dark:bg-slate-700"></span>
                            <span class="text-xs text-slate-400 dark:text-slate-500">{{ $blog->published_at->format('M j, Y') }}</span>
                        </div>
                        ...
                        <p class="text-sm text-slate-500 dark:text-slate-400 line-clamp-3 mb-6 flex-1">{{ $blog->excerpt }}</p>
                        
                        <div class="mt-auto border-t border-gray-100 dark:border-white/5 pt-4">
                            <span class="text-xs font-bold text-teal-600 dark:text-teal-400 uppercase tracking-widest flex items-center gap-1 group-hover:translate-x-1 transition-transform">
                                Explore
