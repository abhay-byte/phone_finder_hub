                @foreach($posts as $post)
                    <li class="group hover:bg-slate-50 transition-colors duration-300">
                        <div class="px-6 py-5">
                            <div class="grid grid-cols-12 gap-4 sm:items-center">
                                ...
                                <div class="col-span-12 sm:col-span-8 lg:col-span-9 flex items-start sm:items-center gap-4">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <a href="{{ route('forum.post.show', $post->slug) }}" class="block focus:outline-none">
                                            <h2 class="text-lg font-bold text-slate-900 group-hover:text-teal-600 transition-colors truncate">
                                                {{ $post->title }}
                                            </h2>
                                        </a>
                                        <div class="mt-1.5 flex items-center gap-3 text-xs text-slate-500">
                                            <div class="flex items-center gap-1.5 font-medium">
                                                <div class="w-4 h-4 rounded-full bg-teal-100 text-teal-700 flex sm:hidden items-center justify-center font-bold text-[8px]">
                                                    {{ strtoupper(substr($post->user->name, 0, 1)) }}
                                                </div>
                                ...
                                <div class="col-span-12 sm:col-span-4 lg:col-span-3 flex justify-start sm:justify-end gap-5 sm:gap-6 mt-3 sm:mt-0 pl-14 sm:pl-0">
                                    <div class="flex flex-col items-center {{ $post->upvotes > 0 ? 'text-teal-600' : '' }}">
                                        <span class="text-sm font-semibold text-slate-800 {{ $post->upvotes > 0 ? '!text-teal-600' : '' }}">{{ number_format($post->upvotes) }}</span>
                                        <span class="text-[10px] text-slate-400 uppercase tracking-widest mt-0.5 {{ $post->upvotes > 0 ? '!text-teal-600' : '' }}">Upvotes</span>
                                    </div>
                                    <div class="flex flex-col items-center">
                                        <span class="text-sm font-semibold text-slate-800">{{ number_format($post->comments_count) }}</span>
                                    ...
                                    <div class="flex flex-col items-center opacity-70">
                                        <span class="text-sm font-medium text-slate-600">{{ number_format($post->views) }}</span>
